...
const inputSchema = z.object({
  ...
  note_path: z
    ...
    .describe(
      ...
      '⚠️ If the user specifies ANY deadline, duration, end date, completion date, or date range (e.g. "before...", "by...", "within...", "finish by...", "complete before...", "22-25 Aug"), the note MUST be treated as spanning an active period. The active period starts from the resolved start date (normally today unless explicitly specified otherwise) and ends at the resolved end date. The filename MUST use the resolved date range. ' +
      'For date ranges within the same month and year, abbreviate the end date to the day only, e.g. "2026-08-22-25-kunming-trip" or "2026-07-20-22-submit-report". If the date range spans different months or years, the end date MUST be written as the full date (YYYY-MM-DD), e.g. "2026-08-05-2026-09-09-finish-asdea" or "2026-08-30-2026-09-02-business-trip". Never abbreviate the end date when the month or year changes. ' +
